...
public class Lane extends Thread implements PinsetterObserver {
	...
	public void partyAssignedAndGameNotFinished() {
		while (gameIsHalted) {
			try {
				sleep(10);
			} catch (Exception e) {}
	...
	public void partyAssignedAndGameFinished() {
	...
	public void run() {
		...
		while (true) {
			if (partyAssigned && !gameFinished) {	// we have a party on this lane, so next bower can take a throw
				partyAssignedAndGameNotFinished();
			} else if (partyAssigned && gameFinished) {
				partyAssignedAndGameFinished();
			}
	...
	public boolean firstBallStrike(int i, int[] curScore) {
	...
	private int getScore( Bowler Cur, int frame) {
		...
		for (int i = 0; i != current+2; i++){
			...
			if( i%2 == 1 && curScore[i - 1] + curScore[i] == 10 && i < current - 1 && i < 19){
				...
				boolean doBreak = firstBallStrike(i, curScore);
				if (doBreak == true)
					break;
			}else {
